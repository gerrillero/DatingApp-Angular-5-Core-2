...
export class UserService {
  ...
  getMessages(id: number, page?: number, itemsPerPage?: number, messageContainer?: string) {
    ...
    return this._authHttp.get(this.baseUrl + 'users/' + id + '/messages' + queryString).map((response: Response) => {
      paginatedResult.result = response.json();
  ...
  getMessageThread(id: number, recipientId: number) {
    return this._authHttp.get(this.baseUrl + 'users/' + id + '/messages/thread/' + recipientId).map((response: Response) => {
      return response.json();
    }).catch(this.handleError);
  }

  sendMessage(id: number, message: Message) {
    return this._authHttp.post(this.baseUrl + 'users/' + id + '/messages', message).map((response: Response) => {
      return response.json();
    }).catch(this.handleError);
  }
